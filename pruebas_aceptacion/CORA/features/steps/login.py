...
@then(
    u'el sistema valida las credenciales y accede al menu donde se muestra "{mensaje}"')
def step_impl(context, mensaje):
    div_mensaje = context.driver.find_element(
        By.XPATH, '/html/body/div/div/div/div/h1')
    assert mensaje in div_mensaje.text, \
        f"El mensaje {mensaje} no se encuentra en {div_mensaje.text}"
    sleep(2)
    context.driver.quit()
