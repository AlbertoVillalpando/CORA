...
class UsuariosViewsExpandedTest(TestCase):
    ...
    def test_login_view_post_invalid_form(self):
        ...
        self.assertTrue(any("Usuario o contraseña incorrectos" in str(m)
                        for m in messages))
    ...
    def test_login_view_user_without_role(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(any("No tienes un rol asignado" in str(m)
                        for m in messages))
    ...
    def test_registro_view_duplicate_email_error(self):
        ...
        self.assertIn('Ya existe Usuario con este Email',
    ...
    def test_vistas_roles_require_login(self):
        ...
        vistas = ['vistaAutor', 'vistaOrganizador',
                  'vistaRevisor', 'vistaAdmin']
    ...
    def test_actualizar_roles_add_role(self):
        ...
        self.client.login(username='administrador@test.com',
                          password='TestPass123!')

        usuario_test = self.usuarios['autor']
        initial_notifications = Notificacion.objects.filter(
            usuario=usuario_test).count()
        ...
        self.assertTrue(usuario_test.groups.filter(
            name='Organizador').exists())

        # Verificar que se creó la notificación
        new_notifications = Notificacion.objects.filter(
            usuario=usuario_test).count()
        ...
        self.assertIn('Se te ha asignado el rol de Organizador',
    ...
    def test_actualizar_roles_remove_role(self):
        ...
        usuario_test.refresh_from_db()
        self.assertFalse(usuario_test.groups.filter(
            name='Organizador').exists())
        ...
        # Verificar contenido de la notificación
        notificacion = Notificacion.objects.filter(usuario=usuario_test).last()
        self.assertIn('Se te ha removido el rol de Organizador',
                      notificacion.mensaje)
    ...
    def test_actualizar_roles_no_change(self):
        ...
        final_notifications = Notificacion.objects.filter(
            usuario=usuario_test).count()
    ...
    def test_responder_invitacion_aceptar(self):
        ...
        self.assertFalse(self.usuarios['autor'].groups.filter(
            name='Revisor').exists())
        ...
        self.assertTrue(self.usuarios['autor'].groups.filter(
            name='Revisor').exists())
    ...
    def test_vista_organizador_with_conferences(self):
        ...
        self.client.login(username='organizador@test.com',
                          password='TestPass123!')
...
class CustomUserModelTest(TestCase):
    ...
    def test_email_unique_constraint(self):
        ...
        # En lugar de esperar IntegrityError, verificar a nivel de
        # modelo/formulario
    ...
    def test_username_unique_constraint(self):
        # Similar approach - verificar a nivel de formulario/vista en lugar de
        # BD
        CustomUser.objects.create_user(**self.user_data)
        ...
        count = CustomUser.objects.filter(
            username=self.user_data['username']).count()
    ...
    def test_save_method_duplicate_email_validation(self):
        ...
        if form.is_valid():
            ...
            if CustomUser.objects.filter(email=email).exists():
                ...
                # El test pasa porque detectó el duplicado
...
class IntegrationTest(TestCase):
    ...
    def test_complete_user_lifecycle(self):
        ...
        registro_response = self.client.post(
            reverse('registro'), registro_data)
        ...
        self.client.logout()
        self.client.login(
            username='admin_integration@test.com', password='adminpass')
