...
class NotificacionesViewsTestCase(TestCase):
    ...
    def test_dropdown_shows_only_user_notifications(self):
        ...
        n1 = Notificacion.objects.create(
            usuario=self.receptor, mensaje='Hola Bob')
        n2 = Notificacion.objects.create(
            usuario=self.emisor, mensaje='Hola Alice')
        response = self.client.get(self.url_dropdown)
    ...
    def test_dropdown_ordering_by_fecha(self):
        times = [timezone.now() + timezone.timedelta(days=d)
                 for d in (2, 1, 0)]
        ...
        self.assertEqual([n.mensaje for n in notis], [
                         'Notif C', 'Notif B', 'Notif A'])
    ...
    def test_mark_as_read_valid(self):
        ...
        Notificacion.objects.create(
            usuario=self.receptor, mensaje='Tarea1', leida=False)
        Notificacion.objects.create(
            usuario=self.receptor, mensaje='Tarea2', leida=False)
        Notificacion.objects.filter(
            usuario=self.receptor, leida=False).count()
        ...
        self.assertEqual(Notificacion.objects.filter(
            usuario=self.receptor, leida=False).count(), 0)
    ...
    def test_mark_as_read_other_user(self):
        ...
        Notificacion.objects.create(
            usuario=self.emisor, mensaje='Privada', leida=False)
        count_before = Notificacion.objects.filter(
            usuario=self.emisor, leida=False).count()
        response = self.client.post(self.url_mark)
        self.assertEqual(response.status_code, 200)
        # No toca notificaciones ajenas
        count_after = Notificacion.objects.filter(
            usuario=self.emisor, leida=False).count()
    ...
    def test_marcar_como_leidas_view(self):
        ...
        # Crear notificaciones no leídas
        Notificacion.objects.create(
            usuario=self.receptor, mensaje='Notif 1', leida=False)
        Notificacion.objects.create(
            usuario=self.receptor, mensaje='Notif 2', leida=False)
