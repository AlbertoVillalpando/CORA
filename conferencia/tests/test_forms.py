...
class ConferenciaFormTestCase(TestCase):
    def setUp(self):
        ...
        self.autor_user = User.objects.create_user(
            ...
            nombre='Autor', apellidos='Test', area_conocimiento='Medicina'
        )
    ...
    def test_init_no_groups(self):
        ...
        self.assertQuerySetEqual(
            form.fields['organizador'].queryset, [], transform=repr)
        self.assertQuerySetEqual(
            form.fields['autor'].queryset, [], transform=repr)
    ...
    def test_invalid_missing_nombre(self):
        Group.objects.create(name='Organizador').user_set.add(
            self.organizador_user)
    ...
    def test_invalid_missing_categoria(self):
        Group.objects.create(name='Organizador').user_set.add(
            self.organizador_user)
        Group.objects.create(name='Autor').user_set.add(self.autor_user)
        data = self.valid_data.copy()
        data.pop('categoria')  # Remove categoria field
        data['organizador'] = self.organizador_user.pk
        data['autor'] = self.autor_user.pk
        form = ConferenciaForm(data=data)
        self.assertFalse(form.is_valid())
