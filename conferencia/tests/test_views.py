...
class ConferenciaViewTestCase(TestCase):
    def setUp(self):
        ...
        self.organizador = User.objects.create_user(
            username='org', password='pass', email='org@example.com')
        self.autor = User.objects.create_user(
            username='aut', password='pass', email='aut@example.com')
        ...
        self.revisor = User.objects.create_user(
            username='revisor', password='1234', email='revisor@example.com')
    ...
    def test_conferencias_list_view_authenticated(self):
        ...
        self.assertTemplateUsed(
            response, 'conferencia/conferencias_autor.html')
    ...
    def test_conferencia_create_view_post_valid(self):
        ...
        self.assertTrue(Conferencia.objects.filter(
            nombre='Nueva Conf').exists())
    ...
    def test_subir_documento_zip_valido(self):
        ...
        archivo = SimpleUploadedFile(
            'doc.zip', b'data', content_type='application/zip')
    ...
    def test_subir_documento_invalido_wrong_type(self):
        url = reverse('subir_documentos', args=[self.conferencia.pk])
        archivo = SimpleUploadedFile(
            'doc.txt', b'data', content_type='text/plain')
        response = self.client.post(url, {'archivo': archivo})
        self.assertRedirects(response, url)
    ...
    def test_subir_documento_extension_invalida(self):
        url = reverse('subir_documentos', args=[self.conferencia.pk])
        archivo = SimpleUploadedFile(
            'archivo.txt', b'data', content_type='text/plain')
        response = self.client.post(url, {'archivo': archivo})
        self.assertEqual(response.status_code, 302)  # redirige a sí mismo

    def test_editar_conferencia_post_invalido(self):
        url = reverse('editar_conferencia', args=[self.conferencia.pk])
        data = {'nombre': '', 'meses': '', 'dias': '', 'horas': '',
                'minutos': '', 'organizador': '', 'autor': ''}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'conferencia/editar.html')
    ...
    def test_ver_invitaciones_conferencia(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'conferencia/invitaciones_conferencia.html')

    def test_responder_invitacion_aceptar(self):
        invitacion = InvitacionRevisor.objects.create(
            conferencia=self.conferencia, autor=self.autor)
    ...
    def test_conferencias_administrador_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'conferencia/conferencias_administrador.html')
        self.assertIn('conferencias', response.context)
    ...
    def test_conferencias_revisor_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'conferencia/conferencias_revisor.html')
    ...
    def test_autores_disponibles_view(self):
        ...
        otro_autor = User.objects.create_user(
            username='otro', password='pass', email='otro@example.com')
